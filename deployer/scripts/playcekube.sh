...
Available Commands:
  ...
  addon         PlayceKube addon command

Usage:
  playcekube [subcommand] [options]
EOF
}
...
PlayceKube Kubernetes controls
...
PlayceKube Kubernetes cluster installer
...
PlayceKube addon controls

Available Commands:
  list              PlayceKube addon list
  install-list      PlayceKube addon installed list
...
function playcekube_k8s_install()
{
  ...
  if [[ "${SUBPARAMS}" == "" ]] || [[ "${SUBPARAMS}" =~ "--help" ]] ; then
    ...
    ${PLAYCE_DIR}/playcekube/deployer/scripts/playcekube_kubespray.sh ${SUBPARAMS}
  fi
}
